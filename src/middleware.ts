...
export async function middleware(request:NextRequest){
 ...
 if(request.nextUrl.pathname.startsWith('/api/')&&!request.nextUrl.pathname.startsWith('/api/market/')&&!request.nextUrl.pathname.startsWith('/api/stripe/')&&!(request.nextUrl.pathname==='/api/products'&&request.method==='GET'))return NextResponse.json({error:'Esta operación pertenece a la versión anterior. Actualiza la página.'},{status:410});
