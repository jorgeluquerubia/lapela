...
export async function handle(request:Request,path:string[]){try{
 ...
 if(request.method==='GET'){
  ...
  if(action==='notifications'){const notes=await result(db.from('lp_notifications').select('*,listing:lp_listings(title)').eq('user_id',user.id).eq('read',false).order('created_at',{ascending:false}));return ok({notifications:notes||[],unreadCount:(notes||[]).length});}
